...
compute_icc_table <- function(model, caption) {
  tmp1 <- performance::icc(model)
  icc_values <- tmp1$ICC  # Extract ICC values
  
  knitr::kable(data.frame(
    "f" = c("Text", "Family", "Language", "Speaker"),
    "ICC" = c(icc_values[1], icc_values[2], icc_values[3], icc_values[4])
  ), 
  digits = 2, row.names = FALSE, col.names = c("Level-1 factor (f)", "ICC"),
  caption = caption)
}

# Compute and display ICCs for each model
compute_icc_table(lmer(NS ~ 1 + (1 | Text) + (1 | Family/Language) + (1 | Speaker), data = info.rate.data),
                  "ICC for model `NS ~ 1 + (1 | Text) + (1 | Family/Language) + (1 | Speaker)`.")

compute_icc_table(lmer(SR ~ 1 + (1 | Text) + (1 | Family/Language) + (1 | Speaker), data = info.rate.data),
                  "ICC for model `SR ~ 1 + (1 | Text) + (1 | Family/Language) + (1 | Speaker)`.")

compute_icc_table(lmer(ShIR ~ 1 + (1 | Text) + (1 | Family/Language) + (1 | Speaker), data = info.rate.data),
                  "ICC for model `ShIR ~ 1 + (1 | Text) + (1 | Family/Language) + (1 | Speaker)`.")

compute_icc_table(lmer(IR ~ 1 + (1 | Text) + (1 | Family/Language) + (1 | Speaker), data = info.rate.data),
                  "ICC for model `IR ~ 1 + (1 | Text) + (1 | Family/Language) + (1 | Speaker)`.") 
...
Bescause there is missing data from *Age*, and because the GAMLSS models require no missing data, we will fit the models with *Age* (and its interaction with *Sex*) on the subset of the data that contains only those speakers with *Age* info.
...
As above, we also looked at the simple lmer model:
...
When modelling the *relationship* between *SR* and *ID*, this *negative relationship*:
  ...
  - seems **strengthened** with increasing *Age* (i.e., older speakers of both sexes show a stronger negative relationship between *SR* and *ID*), and 
...
Here we test the hypothesis that *ID* is confounded by *Age* and *Sex* structures between languages:

```{r ID_age_sex, echo=FALSE}
model.ID.age.sex <- lmer(ID ~ 1 + Sex * Age + (1 | Family), data=info.rate.data.for.age);
model.ID.age     <- update(model.ID.age.sex, . ~ . - Sex:Age - Sex); anova(model.ID.age, model.ID.age.sex);
model.ID         <- update(model.ID.age, . ~ . - Age); anova(model.ID, model.ID.age.sex);

summary(model.ID.age.sex);
```
...
In our case, SDIR (versus *Vietnamese*) is reduced to the ratio between the number of syllables (*NS*) in *Vietnamese* and the *NS* in the language *L* for each *Text* separately; we denote this here also as *NSVR* (from "NS Vietnamese Ratio").

### *SDIR* separately for each *Text* and *Language*

```{r ID vs SDIR, message = FALSE, echo=FALSE, fig.width=9, fig.height=10, fig.cap="Syntagmatic density of information ratio *SDIR* (relative to Vietnamese) versus *ID* with LOESS smoother (black) and linear regression (yellow) and their 95%CIs."}
# The data: just the Text-level info:
d <- unique(info.rate.data[,c("Text", "Language", "Family", "ID", "NSVR")]);

# Plot:
ggplot(d, aes(x=ID, y=NSVR, color=Family)) + 
  geom_point(alpha=0.50, size=1) + 
  geom_smooth(method="lm", color="yellow") +
  geom_smooth(method="loess", color="black") + 
  geom_label_repel(aes(x=ID, y=x, label=Language, fill=Family), color="black", alpha=0.5, size=3, show.legend = FALSE, 
                   data=aggregate(d$NSVR, by=d[c("ID","Language","Family")], max)) +
  ylab("SDIR (relative to Vietnamese)") + xlab("ID") + 
...
cor.test(d$NSVR, d$ID, method="pearson");
cor.test(d$NSVR, d$ID, method="spearman");
```

The multi-level regression of *SDIR* on *ID* (with *Text* as random effect) and the *Text*'s ICC are:
```{r message = FALSE, echo=FALSE}
m0 <- lmer(NSVR ~ ID + (1 | Text), data=d);
summary(m0);
performance::icc(m0);
# original: icc(m0);
```
...
### Average *SDIR* across *Text*s for each *Language*

If we consider just the **average *SDIR* ** for each language:

```{r ID vs average SDIR, message = FALSE, echo=FALSE, fig.width=9, fig.height=10, fig.cap="Average syntagmatic density of information ratio *SDIR* (relative to Vietnamese) versus *ID* with LOESS smoother (black) and linear regression (yellow) and their 95%CIs."}
# The data: just the Text-level info:
d <- unique(info.rate.data[,c("Text", "Language", "Family", "ID", "NSVR")]);
d1 <- aggregate(d$NSVR, by=d[c("Language", "Family", "ID")], mean); names(d1)[ncol(d1)] <- "NSVR.mean";

# Plot:
ggplot(d1, aes(x=ID, y=NSVR.mean, color=Family)) + 
  geom_point(alpha=0.50, size=1) + 
  geom_smooth(method="lm", color="yellow") +
  geom_smooth(method="loess", color="black") + 
  geom_label_repel(aes(x=ID, y=NSVR.mean, label=Language, fill=Family), color="black", alpha=0.5, size=3, show.legend = FALSE) +
  ylab("Average SDIR (relative to Vietnamese)") + xlab("ID") + 
  theme(legend.position="bottom") +
  NULL;

cor.test(d1$NSVR.mean, d1$ID, method="pearson");
cor.test(d1$NSVR.mean, d1$ID, method="spearman");

m1 <- lm(NSVR.mean ~ ID, data=d1);
summary(m1);
```
...
In what follows, mixing probabilities is done independently from factors such as Sex.
...
For each such test, we performed four randomisation procedures to obtain an estimate of the "specialness" of the observed unimodality estimate; for each new permuted dataset, we recomputed everything before estimating the unimodality of the permuted distribution:
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=9, fig.width=6, fig.cap="**Permutation of the SRs of the texts (PM1)**."}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=9, fig.width=6, fig.cap="**Permutation of the IDs of the languages (PM2)**."}
# Plotting:
par(mfrow=c(3,2));
alpha.level = 0.05;
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=9, fig.width=6, fig.cap="**Permutation of the average SRs of the languages with speaker adjustement (PM4)**."}
# Plotting:
par(mfrow=c(3,2));
alpha.level = 0.05;
...
We compute various distances between languages (as implemented by the `distance()` function in package `philentropy`) in what concerns the distribution of NS, SR and ID.
...
Here we compare the Speech Rate (*SR*) used in the paper and defined as the *canonical articulatory rate* (= the number of syllables corresponding to the canonical text pronunciation per second of speech) with an estimate of the *realized speech rate*, based on the *automatic detection* of syllable nuclei implemented by the popular algorithm described in De Jong, Nivja H., and Ton Wempe. "Praat script to detect syllable nuclei and measure speech rate automatically." Behavior research methods 41.2 (2009): 385-390.
We used their algorithm with standard parameters (except for pauses which were retrieved from our manual annotation to match the main analysis) on the actual oral productions of our speakers, and the results are available in the TAB-separated CSV file `AutomaticSylDetect.csv` with the following structure:
...
```{r read and parse automatic syllable detection, echo=FALSE, warning=FALSE, message=FALSE}
# Load the automatic syllabification data:
syll.data <- read.table("./AutomaticSylDetect.csv", header=TRUE, sep="\t", quote="", strip.white=TRUE, stringsAsFactors=FALSE);

# Extract language and text (and keep the minimal speaker ID only):
syll.data <- cbind(do.call(rbind, lapply(strsplit(syll.data$soundname,"_",fixed=TRUE), function(x)
                    {
                      if( length(x) >= 5 )
                      {
                        data.frame("Speaker"=paste0(x[1],"_",x[2],"(",x[3],")"), "Language"=toupper(x[1]), "Text"=toupper(x[5]));
                      } else if( length(x) == 3 )
                      {
                        data.frame("Speaker"=paste0(x[1],"_",x[2]), "Language"=toupper(x[1]), "Text"=toupper(x[3])); # special case for DEU, ENG and ITA
                      } else
                      {
                        stop("Possible error in the AutomaticSylDetect.csv file...\n");
                      }
                    })),
                   "Duration"=syll.data$phonationtime, 
                   "NS"=syll.data$nsyll
                  );

# Merge to the canonical SR data:
syll.data <- merge(syll.data, info.rate.data, by=c("Speaker", "Language", "Text"), all.x=TRUE, suffixes=c(".auto",""));

# Compute SR and IR:
syll.data$SR.auto <- (syll.data$NS.auto / syll.data$Duration.auto);
syll.data$IR.auto <- (syll.data$SR.auto * syll.data$ID);

# Sort the data:
syll.data$Language <- as.character(syll.data$Language);
syll.data$Speaker <- as.character(syll.data$Speaker);
syll.data$Text <- as.character(syll.data$Text);
syll.data <- syll.data[ order(syll.data$Language, syll.data$Speaker, syll.data$Text), ];

# Convert to factors:
syll.data$Text     <- as.factor(syll.data$Text);
syll.data$Speaker  <- as.factor(syll.data$Speaker);
syll.data$Language <- as.factor(syll.data$Language);
syll.data$Family   <- as.factor(syll.data$Family);
syll.data$Sex      <- as.factor(syll.data$Sex);
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
cor.test(syll.data$NS, syll.data$NS.auto, method="pearson");
cor.test(syll.data$NS, syll.data$NS.auto, method="spearman");
t.test(syll.data$NS, syll.data$NS.auto, paired=TRUE);
```

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=7, fig.height=6, fig.cap="NS: canonical (x axis) vs automatic (y axis) overall (black) and separately by language (colored)."}
ggplot(syll.data, aes(x=NS, y=NS.auto, color=Language)) +
  geom_point() + 
  geom_smooth(method="lm", formula=y ~ x, se=FALSE) + 
  geom_smooth(aes(x=NS, y=NS.auto), color="black", method="lm", formula=y ~ x, se=FALSE);
```

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=7, fig.height=6, fig.cap="NS: canonical (x axis) vs automatic (y axis) overall (black) and separately by text (colored)."}
ggplot(syll.data, aes(x=NS, y=NS.auto, color=Text)) +
  geom_point() + 
  geom_smooth(method="lm", formula=y ~ x, se=FALSE) + 
  geom_smooth(aes(x=NS, y=NS.auto), color="black", method="lm", formula=y ~ x, se=FALSE);
```

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=15, fig.height=14, fig.cap="NS: canonical (x axis) vs automatic (y axis) separately by text and language."}
ggplot(syll.data, aes(x=NS, y=NS.auto, color=Language)) +
  geom_point() + 
  geom_smooth(method="lm", formula=y ~ x, se=FALSE, color="black") +
  facet_grid(Language ~ Text);
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
cor.test(syll.data$SR, syll.data$SR.auto, method="pearson");
cor.test(syll.data$SR, syll.data$SR.auto, method="spearman");
t.test(syll.data$SR, syll.data$SR.auto, paired=TRUE);
```

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=7, fig.height=6, fig.cap="SR: canonical (x axis) vs automatic (y axis) overall (black) and separately by language (colored)."}
ggplot(syll.data, aes(x=SR, y=SR.auto, color=Language)) +
  geom_point() + 
  geom_smooth(method="lm", formula=y ~ x, se=FALSE) + 
  geom_smooth(aes(x=SR, y=SR.auto), color="black", method="lm", formula=y ~ x, se=FALSE);
```

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=7, fig.height=6, fig.cap="SR: canonical (x axis) vs automatic (y axis) overall (black) and separately by text (colored)."}
ggplot(syll.data, aes(x=SR, y=SR.auto, color=Text)) +
  geom_point() + 
  geom_smooth(method="lm", formula=y ~ x, se=FALSE) + 
  geom_smooth(aes(x=SR, y=SR.auto), color="black", method="lm", formula=y ~ x, se=FALSE);
```

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=15, fig.height=14, fig.cap="SR: canonical (x axis) vs automatic (y axis) separately by text and language."}
ggplot(syll.data, aes(x=SR, y=SR.auto, color=Language)) +
  geom_point() + 
  geom_smooth(method="lm", formula=y ~ x, se=FALSE, color="black") +
  facet_grid(Language ~ Text);
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=13, fig.height=14, fig.cap="SR: canonical (x axis) vs automatic (y axis) separately by speaker."}
ggplot(syll.data, aes(x=SR, y=SR.auto, color=Text)) +
  geom_point() + 
  geom_smooth(method="lm", formula=y ~ x, se=FALSE, color="black") +
  facet_wrap(. ~ Speaker);
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=13, fig.height=14, fig.cap="SR: canonical (x axis) vs automatic (y axis) separately by language with regression line (black) and LOESS smoothing (yellow) and their 95%CIs."}
ggplot(syll.data, aes(x=SR, y=SR.auto, color=Language)) +
  geom_point(alpha=0.5) + 
  geom_smooth(method="lm", formula=y ~ x, se=TRUE, color="black", alpha=0.5) + 
  geom_smooth(method="loess", se=TRUE, color="yellow", alpha=0.5) + 
  #geom_smooth(aes(x=SR, y=SR.auto), color="black", method="lm", formula=y ~ x, se=FALSE) + 
  facet_wrap(. ~ Language) + 
  xlab("SR (canonical)") + ylab("SR (automatic)");
```

**Across languages:**
```{r echo=FALSE, warning=FALSE, message=FALSE}
m0 <- lmer(SR.auto ~ SR + (1 | Family/Language) + (1 | Text) + (1 | Speaker), data=syll.data); m0s <- summary(m0); print(m0s);
performance::icc(m0);
r1 <- cor.test(syll.data$SR, syll.data$SR.auto, method="pearson"); #print(r1);
r2 <- cor.test(syll.data$SR, syll.data$SR.auto, method="spearman"); #print(r2)
t1 <- t.test(syll.data$SR, syll.data$SR.auto, paired=TRUE); #print(t1);

# Collect the results:
results <- data.frame("Language"="all",
                      "r"=paste0("r=",sprintf("%.2f",r1$estimate), " (p=",sprintf("%.4g",r1$p.value),")"),
                      "rho"=paste0("rho=",sprintf("%.2f",r2$estimate), " (p=",sprintf("%.4g",r2$p.value),")"),
                      "t"=paste0("t(",sprintf("%.1f",t1$parameter),")=",sprintf("%.2f",t1$statistic), " (p=",sprintf("%.3g",t1$p.value),")"),
                      "intercept"=paste0(sprintf("%.2f",m0s$coefficients["(Intercept)","Estimate"])," (p=",sprintf("%.3g",m0s$coefficients["(Intercept)","Pr(>|t|)"]),")"),
                      "slope"=paste0(sprintf("%.2f",m0s$coefficients["SR","Estimate"])," (p=",sprintf("%.3g",m0s$coefficients["SR","Pr(>|t|)"]),")"));
```

**For each language separately:**
```{r echo=FALSE, warning=FALSE, message=FALSE}
for( i in unique(syll.data$Language) )
{
  cat(paste0("\nFor *",i,"*\n\n"));
  d <- syll.data[ syll.data$Language == i, ];
  m0 <- lmer(SR.auto ~ SR + (1 | Text) + (1 | Speaker), data=d);
  m0s <- summary(m0); print(m0s);
  print(performance::icc(m0))
  # print(icc(m0));
  r1 <- cor.test(d$SR, d$SR.auto, method="pearson"); #print(r1);
  r2 <- cor.test(d$SR, d$SR.auto, method="spearman"); #print(r2);
  t1 <- t.test(d$SR, d$SR.auto, paired=TRUE); #print(t1);
  
  # Collect the results:
  results <- rbind(results,
                   data.frame("Language"=i,
                              "r"=paste0("r=",sprintf("%.2f",r1$estimate), " (p=",sprintf("%.4g",r1$p.value),")"),
                              "rho"=paste0("rho=",sprintf("%.2f",r2$estimate), " (p=",sprintf("%.4g",r2$p.value),")"),
                              "t"=paste0("t(",sprintf("%.1f",t1$parameter),")=",sprintf("%.2f",t1$statistic), " (p=",sprintf("%.3g",t1$p.value),")"),
                              "intercept"=paste0(sprintf("%.2f",m0s$coefficients["(Intercept)","Estimate"])," (p=",sprintf("%.3g",m0s$coefficients["(Intercept)","Pr(>|t|)"]),")"),
...
knitr::kable(results, row.names=FALSE, 
             col.names=c("Language", "Pearson's r", "Spearman's rho", "Paired t-test", "Intercept", "Slope"), 
             caption="Correlations (Pearson's and Spearman's), paired t-test, and (mixed-effects, with Text, Speaker and, for the first row only, Language, as random effects) linear regression intercept and slope between automatic and canonical SR across all languages (top row) and for each language spearately).");
```
...
tiff("./figures/Figure-SDIR-vs-ID.tiff", width=6, height=7, units="in", compression="lzw", res=600, type="cairo");
d <- unique(info.rate.data[,c("Text", "Language", "Family", "ID", "NSVR")]);
ggplot(d, aes(x=ID, y=NSVR, color=Family)) + 
  geom_point(alpha=0.50, size=1) + 
  geom_smooth(method="lm", color="yellow") +
  ...
  geom_label_repel(aes(x=ID, y=x, label=Language, fill=Family), color="black", alpha=0.5, size=3, show.legend = FALSE, 
                   data=aggregate(d$NSVR, by=d[c("ID","Language","Family")], max)) +
  ylab("SDIR (relative to Vietnamese)") + xlab("ID") + theme(legend.position="bottom") +
  NULL;
dev.off();

tiff("./figures/Figure-SR-can-vs-auto.tiff", width=7, height=8, units="in", compression="lzw", res=600, type="cairo");
ggplot(syll.data, aes(x=SR, y=SR.auto, color=Language)) +
  geom_point(alpha=0.5) + 
  geom_smooth(method="lm", formula=y ~ x, se=TRUE, color="black", alpha=0.5) + 
  #geom_smooth(method="loess", se=FALSE, color="yellow", alpha=0.5) + 
  #geom_smooth(aes(x=SR, y=SR.auto), color="black", method="lm", formula=y ~ x, se=FALSE) + 
  facet_wrap(. ~ Language) + 
  xlab("SR (canonical)") + ylab("SR (automatic)") + theme(legend.position="bottom");
dev.off();
```
